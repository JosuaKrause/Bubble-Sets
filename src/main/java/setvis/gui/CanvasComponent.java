...
public class CanvasComponent extends JComponent implements Canvas {
  ...
  public final class Position {
     ...
     * @param rect The reference to the rectangle.
     */
    Position(final int groupID, final Rectangle2D rect) {
   ...
   * 
   * @param shaper The shape generator for the outlines.
   */
  ...
  public void setShapeAndOutline(final OutlineType outlineType,
    ...
    if(oldType == outlineType) {
      ...
      if(oldShapeType == shapeType) return;
      outline = oldOut;
    } else {
      switch(outlineType) {
        case BUBBLE_SETS:
   ...
   * 
   * @param changes The changes of the canvas as defined in
  ...
  /**
   * Signalizes that something has changed. This results in clearing the outline
   * cache, notifying the parent and a call to {@link #repaint()}.
   * 
   * @param changes The changes of the canvas as defined in
   *          {@link CanvasListener#canvasChanged(int)}.
   */
  ...
  /**
   * Adds a new item to the canvas. The parameters are chosen from the current
   * state.
   * 
   * @param tx The horizontal position.
   * @param ty The vertical position.
   */
  public void addItem(final double tx, final double ty) {
    addItem(curItemGroup, tx, ty, curItemWidth, curItemHeight);
  }
  ...
  public void paint(final Graphics gfx) {
    ...
    if(groupShapes == null) {
      ...
      final AbstractShapeGenerator shaping = new ShapeSimplifier(shaper,
          simplifyTolerance);
      groupShapes = shaping.createShapesForLists(items);

...
public enum OutlineType {
   ...
   * @param assocClass The associated class. It is used to reverse lookup types
   *          from {@link SetOutline} objects.
   */
  ...
  /** The lookup map. */
  private static final Map<Class<?>, OutlineType> MAP =
      new HashMap<Class<?>, OutlineType>();
   ...
   * 
   * @param set The outline object to find the type for.
  ...
  private AbstractOutlineConfiguration createBubbleSetConfiguration(
    ...
    return new AbstractOutlineConfiguration(canvas, this) {
      ...
      /**
       * Whether this component is in update mode. When this component is in the
       * update mode, the changes are assumed to be outside and no changes to
       * the underlying bubble set are made.
       */
       ...
       * Getter.
       * 
       * @return Whether this component performs a text update.
       */
      ...
      /**
       * Creates almost a copy of the given bubble set. Only iterations are
       * changed.
       * 
       * @param bubble The set to copy.
       * @return The new bubble set.
       */
      public BubbleSet createBubbleSet(final BubbleSet bubble) {
        return new BubbleSet(routingIt.getValue(), marchingIt.getValue(),
            bubble.getPixelGroup(), bubble.getEdgeR0(),
            bubble.getEdgeR1(), bubble.getNodeR0(), bubble.getNodeR1(),
            bubble.getMorphBuffer(), bubble.getSkip());
      }

      /**
       * Sets the values for the bubble set.
       * 
       * @param bubble The bubble set.
       */
      public void setValues(final BubbleSet bubble) {
  ...
  /**
   * Creates the java constructor text.
   * 
  ...
  public static void creationText(final SetOutline outline,
    ...
    if(outline instanceof BubbleSet) {
      ...
      if(BubbleSet.DEFAULT_EDGE_R0 == b.getEdgeR0()
          && BubbleSet.DEFAULT_EDGE_R1 == b.getEdgeR1()
          && BubbleSet.DEFAULT_MAX_MARCHING_ITERATIONS == b.getMaxMarchingIterations()
          && BubbleSet.DEFAULT_MAX_ROUTING_ITERATIONS == b.getMaxRoutingIterations()
          && BubbleSet.DEFAULT_MORPH_BUFFER == b.getMorphBuffer()
          && BubbleSet.DEFAULT_NODE_R0 == b.getNodeR0()
          && BubbleSet.DEFAULT_NODE_R1 == b.getNodeR1()
          && BubbleSet.DEFAULT_PIXEL_GROUP == b.getPixelGroup()
          && BubbleSet.DEFAULT_SKIP == b.getSkip()) {
        sb.append(")");
        return;
      }

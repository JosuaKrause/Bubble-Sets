...
public final class VecUtil {
    ...
    public static final double distPointLineSqr(final Point2D start,
        ...
        final Point2D move = subVec(end, start);
        final double div = vecLengthSqr(move);
        if (div == 0) {
            return vecLengthSqr(subVec(start, p));
        }
        final double x1 = start.getX();
        final double y1 = start.getY();
        final double x2 = end.getX();
        final double y2 = end.getY();
        final double x3 = p.getX();
        final double y3 = p.getY();
        final double u = (x3 - x1) * (x2 - x1) + (y3 - y1) * (y2 - y1);
        if (u >= 1) {
            return vecLengthSqr(subVec(end, p));
        }
        if (u <= 0) {
            return vecLengthSqr(subVec(start, p));
        }
        final Point2D inter = addVec(start, mulVec(move, u / div));
        return vecLengthSqr(subVec(p, inter));
    }

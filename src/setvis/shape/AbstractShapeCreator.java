...
public abstract class AbstractShapeCreator {
	...
	private boolean isAutoParallel() {
		return AUTO_PARALLEL && THREAD_COUNT > 1;
	}

	private boolean isSingletonThreadCount() {
		return THREAD_COUNT < 2;
	}

	private boolean isInvalidThreadCount() {
	...
	public final Shape[] createShapesFor(final Collection<Rectangle2D[]> items) {
		...
		if (setOutline instanceof BubbleSet) {
			final BubbleSet bset = (BubbleSet) setOutline;
			bset.setSharedStatus(true);
		}
	...
	public final Shape[] createShapesInParallel(final Group[] groups) {
		if (isSingletonThreadCount()) {
			if (isInvalidThreadCount()) {
